...
PJsonReader::parseInternal( const string& contents, int& index )
{
	...
	while( index < contents.length() ) {
		...
		//cout << "The next 100 characters after adding child: " << endl;
		//for( int i=0; i<100; i++ ) {
...
PJsonReader::getNodeType( const char& nodeStart )
{
	...
	switch( tolower( nodeStart ) ) {
		...
		case 'n':
			return NULLOBJ;
		case '}':
			cout << "Found a closing brace" << endl;
	}
}
...
PJsonReader::parseName( const string& contents, int& index )
{
	...
	index = goToNextDelimiter( contents, index );
	char ch;
	string name;
	while( 1 ) {
		ch = contents[++index];
...
PJsonReader::parseString( const string& contents, int& index )
{
	...
	goToNextDelimiter( contents, index );
...
int
PJsonReader::goToNextDelimiter( const string& contents, int& index )
{
	cout << "==== GO TO NEXT DELIMITER ====" << endl;
	cout << "-> Starting at " << contents[index] << endl;
	for( int i = index; i < contents.length(); i++ ) {
		cout << "-> ..character: " << contents[i] << endl;
		if( contents[i] == '"' ) {
			cout << "\tfound a \", eating whitespace" << endl;
			eatWhiteSpace( contents, index );
			cout << "\teatWhiteSpace() done, character is now: " << contents[i+1] << " (returning)" << endl;
			return i;
		} else if( contents[i] == '}' ) {
			cout << "\tfound a }, we want to leave the index here (contents[i] = " << contents[i] << endl;
			return i;
		} else if( contents[i] == ',' ) {
			cout << "\tfound a ',' - eating whiespace" << endl;
			eatWhiteSpace( contents, index );
			cout << "\teatWhiteSpace() done, character is now: " << contents[i+1] << " (returning)" << endl;
			return i;
		}
